...
export default function StoryDetail({ story }: StoryDetailProps) {
  ...
  const title =
    typeof rawTitle === 'string' || typeof rawTitle === 'number' ? rawTitle : 'Untitled Story';
  const rawCreated = story.Created || story.createdTime;
  const created =
    typeof rawCreated === 'string' || typeof rawCreated === 'number' ? rawCreated : '';
  const location = Array.isArray(story['Location (from Media)'])
      ...
      {images.length > 0 && (
        <section className="mb-8">
          <div className="flex flex-wrap gap-4 justify-center">
            {images.map(
              (img: { url?: string; width?: number; height?: number } | string, idx: number) => {
